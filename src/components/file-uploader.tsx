  ...
  const onDrop = React.useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      if (!multiple && maxFileCount === 1 && acceptedFiles.length > 1) {
        toast.error("Нельзя загрузить больше одного файла");
        return;
      }

      if ((files?.length ?? 0) + acceptedFiles.length > maxFileCount) {
        toast.error(`Слишком много файлов: ${maxFileCount}`);
        return;
      }
      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ file }) => {
          toast.error(`Файл ${file.name} невалидный`);
        });
      }
          ...
          >
            ...
            {isDragActive ? (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                ...
                <p className="font-medium text-muted-foreground">Перетащите сюда изображение</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                <div className="rounded-full border border-dashed p-3">
                ...
                <div className="flex flex-col gap-px">
                  ...
                  <p className="text-sm text-muted-foreground/70">
                    ...
                    {maxFileCount > 1
                      ...
                      : ` один файл размера ${formatBytes(maxSize)}`}
                  </p>
                </div>
              </div>
            )}
